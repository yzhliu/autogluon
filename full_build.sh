#!/usr/bin/env bash
set -euo pipefail
cd common && rm -rf build/ dist/* && python3 setup.py clean bdist_wheel && cd ..
cd core && rm -rf build/ dist/* && python3 setup.py clean bdist_wheel && cd ..
cd features && rm -rf build/ dist/* && python3 setup.py clean bdist_wheel && cd ..
cd tabular && rm -rf build/ dist/* && python3 setup.py clean bdist_wheel && cd ..
...
cd autogluon && rm -rf build/ dist/* && python3 setup.py clean bdist_wheel && cd ..
...
DIST_DST=/Users/yizhiliu/Workspace/autogluon_top/web-deploy/dist
